"""Verify the exact distributable bytes against a successful native creative test."""
...
manifests={kind:json.loads((r/f'src-tauri/{kind}-runtime.json').read_text())['files'] for kind in ['image','video']}
...
result=dict(passed=True,version=v,nativeReport=str(native.relative_to(r)),executableSha256=exe,zipSha256=sha(archive),installerSha256=sha(installer),zipFiles=len(expected),runtimeFiles={k:len(f) for k,f in manifests.items()},specAndPromptSha256=spec)
