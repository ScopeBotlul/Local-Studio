  ...
  return <div className="page hub-page">
    ...
    {mode === 'hub' ? website ? <HfBrowserPanel language={language} signingIn={!!auth?.pending} onModel={repo => showModels(repo)} /> : <>
      ...
      {detail ? <section className="panel hub-detail" aria-label={t.details}>
        ...
        <div className="hub-tags"><span className="pill">{detail.model.gated ? t.gated : detail.model.private ? t.private : t.public}</span><span className="pill">{t.license}: {detail.model.license ?? t.unknown}</span>{detail.model.library && <span className="pill">{detail.model.library}</span>}</div>
        ...
        <div className="hub-results" aria-live="polite">{models.length === 0 ? <p>{t.empty}</p> : models.map(model => <article className="panel hub-model" key={model.id}><div className="hub-model-icon"><Box size={21} /></div><div className="hub-model-content"><h2>{model.id}</h2><p className="model-size" data-testid="model-size" title={t.repoSizeHint}>{t.repoSize}: <strong>{sizes[`${model.id}@${model.revision}`] === undefined ? t.working : formatGigabytes(sizes[`${model.id}@${model.revision}`], language)}</strong></p><div className="hub-model-meta">{model.task && <span>{model.task}</span>}<span>{t.license}: {model.license ?? t.unknown}</span>{model.gated && <span>{t.gated}</span>}{model.private && <span>{t.private}</span>}</div><small>{new Intl.NumberFormat(language).format(model.downloads)} {t.downloads} · {new Intl.NumberFormat(language).format(model.likes)} {t.likes}</small></div><button className="button secondary" disabled={busy} onClick={() => void inspect(model.id)}>{t.details}<ArrowRight size={14} /></button></article>)}</div>
