...
export default function App() {
  ...
  const [studioTab,setStudioTab]=useState<'generate'|'canvas'|'timeline'>('generate');
  useEffect(()=>{if(page!=='studio'||studioTab==='generate')return;return setMenuContext('creative',{priority:1,actions:{...(creative.canUndo?{undo:creative.undo}:{}),...(creative.canRedo?{redo:creative.redo}:{})}});},[page,studioTab,creative.canUndo,creative.canRedo,creative.data]);
  useEffect(()=>{const handler=(e:KeyboardEvent)=>{if(!snapshot||page!=='studio'||studioTab==='generate'||document.querySelector('dialog[open]')||(e.target instanceof HTMLElement&&e.target.closest('input,textarea,select,[contenteditable=true]')))return;const action=shortcutFor(e,snapshot.settings.shortcuts);if(action==='undo'){e.preventDefault();creative.undo();}if(action==='redo'){e.preventDefault();creative.redo();}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler);},[page,studioTab,creative.data,snapshot?.settings.shortcuts]);
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell" inert={exitBusy}><header className="topbar"><div className="breadcrumb">Local Studio<span>/</span><strong>{pageTitle}</strong></div><button onClick={() => setPage('jobs')} className="topbar-status" title={pollError ? t.connectionLost : t.monitor}><span className={`connection-dot ${pollError ? 'warning' : ''}`} />{activeCount ? `${activeCount} ${t.active}${activeImages.length ? ' · Image' : ''}` : t.core}</button></header>
      <main id="main-content" className="main-content">
        <ProjectPanel shortcuts={snapshot.settings.shortcuts} maxUndo={snapshot.settings.maxUndo} controller={projects} language={language} disabled={!studio.ready || studio.recovery || saving} changed={!!projects.project && JSON.stringify(projects.project.request) !== JSON.stringify(studio.request)} />
