...
export default function ImageStudio({ onAddToProject, projectDisabled, shortcuts, language, request, setRequest, selectModel, onRestore, selectedJob, disabled = false, galleryOnly = false }: { onAddToProject: (id: string) => Promise<boolean>; projectDisabled: boolean; shortcuts: Shortcuts; language: Language; request: ImageRequest; setRequest: Dispatch<SetStateAction<ImageRequest>>; selectModel: (path: string) => void; onRestore: (request: ImageRequest) => void; selectedJob?: string | null; disabled?: boolean; galleryOnly?: boolean }) {
  ...
  return <div className="page image-studio" onKeyDown={event => {
    ...
    {!galleryOnly && <fieldset disabled={disabled} className="panel image-config">
      ...
      <button data-image-generate className="button primary" disabled={busy || waiting.length >= 20 || !probe?.ready || !request.prompt.trim()} onClick={() => void act(async () => { const job = await imageApi.generate({ ...request }); setSelected(job.id); })}><ImagePlus size={17} />{busy ? (de ? 'Modell für Auftrag prüfen …' : 'Verifying model for job …') : (active || waiting.length ? (de ? 'Bild einreihen' : 'Queue image') : (de ? 'Bild generieren' : 'Generate image'))}</button>
    </fieldset>}
    {current && <section className="panel image-result" data-testid="image-result">
      ...
      {current.status === 'running' && <><progress max={current.phase === 'hashing' ? current.modelBytes : current.request.steps} value={current.phase === 'hashing' ? current.hashedBytes : current.phase === 'sampling' ? current.step : undefined} aria-label="Image progress" /><p>{current.phase === 'hashing' ? `${formatBytes(current.hashedBytes, language)} / ${formatBytes(current.modelBytes, language)}` : current.phase === 'sampling' ? `${current.step} / ${current.request.steps} ${de ? 'Schritte' : 'steps'}` : (de ? 'Fortschritt für diese Phase nicht messbar.' : 'Progress is not measurable for this phase.')}</p></>}
