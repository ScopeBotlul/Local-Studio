...
export default function StorageMaintenance({ settings, de, disabled, dirty, onChange }: { settings: Settings; de: boolean; disabled: boolean; dirty: boolean; onChange: (patch: Partial<Settings>) => void }) {
  ...
  return <section className="panel settings-section storage-cleanup">
    ...
    <p className="hub-hint">{de ? 'Erfasst bekannte Projektarbeitskopien und temporäre Bildaufträge. Aktive Projekte, die letzten drei Wiederherstellungsstände und ungespeicherte Bilder bleiben geschützt. Modelle, Galerieoriginale, Downloads und unbekannte Dateien werden nicht gelöscht.' : 'Covers registered project working copies and temporary image jobs. Active projects, the latest three recovery points and unsaved images stay protected. Models, gallery originals, downloads and unknown files are never deleted.'}</p>
