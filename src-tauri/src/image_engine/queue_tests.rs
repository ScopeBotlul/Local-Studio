...
fn fixture(engine: &ImageEngine, path: &Path) -> ImageJob {
    ...
    let job = ImageJob { restricted:false,locked:false,batch:None,sampling_steps:None, id: uuid::Uuid::new_v4().to_string(), request, status: "queued".into(), phase: "queued".into(), step: 0, hashed_bytes: 0, model_bytes: 4, model_sha256: Some("immutable test hash".into()), runtime: "unit fixture".into(), device: "unit fixture".into(), created_at: crate::database::now(), elapsed_ms: 0, error: None, output: None, saved_path: None, saved_binding: None, working_directory: None, log_tail: String::new(), discarded: false, started_at: None, finished_at: None, queue_position: None };
