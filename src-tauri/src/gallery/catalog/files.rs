...
#[tauri::command]
pub async fn gallery_file_action(request:FileRequest,core:State<'_,Arc<Core>>,catalog:State<'_,Arc<GalleryCatalog>>,images:State<'_,Arc<ImageEngine>>)->Result<FileReport>{let permit=thumbnails::exclusive().await?;let root=root(&core)?;let cat=catalog.inner().clone();let images=images.inner().clone();tauri::async_runtime::spawn_blocking(move||{let _permit=permit;cat.file_action(&root,request,&images)}).await.map_err(|_|"gallery_storage")?}
#[tauri::command]
pub async fn gallery_trash_list(offset:usize,core:State<'_,Arc<Core>>,catalog:State<'_,Arc<GalleryCatalog>>,images:State<'_,Arc<ImageEngine>>)->Result<TrashList>{let root=root(&core)?;let cat=catalog.inner().clone();let images=images.inner().clone();tauri::async_runtime::spawn_blocking(move||{cat.recover(&root,&images)?;cat.trash_page(&root,offset)}).await.map_err(|_|"gallery_storage")?}
#[tauri::command]
pub async fn gallery_trash_action(request:TrashRequest,core:State<'_,Arc<Core>>,catalog:State<'_,Arc<GalleryCatalog>>,images:State<'_,Arc<ImageEngine>>)->Result<FileReport>{let permit=thumbnails::exclusive().await?;let root=root(&core)?;let cache=PathBuf::from(core.storage_paths()?.cache);let cat=catalog.inner().clone();let images=images.inner().clone();tauri::async_runtime::spawn_blocking(move||{let _permit=permit;if request.action=="purge"&&request.confirmed {thumbnails::clear(&cache)?;}cat.trash_action(&root,request,&images)}).await.map_err(|_|"gallery_storage")?}
#[tauri::command]
pub async fn gallery_trash_detail(id:String,core:State<'_,Arc<Core>>,catalog:State<'_,Arc<GalleryCatalog>>)->Result<Detail>{let root=root(&core)?;let item=catalog.trash_item(&root,&id)?;let path=internal(&root,&item.stored_path)?;if path_identity(&path)?!=item.file_id{return Err("gallery_changed".into());}Ok(Detail{url:format!("http://gallery.localhost/{}/{}",root_id(&root),URL_SAFE_NO_PAD.encode(item.stored_path.as_bytes())),kind:item.kind,request:None,job_id:None,origin:None,dimensions:None})}
