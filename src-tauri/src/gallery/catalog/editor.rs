...
pub(crate) fn render_rgba(file:File,operations:&[Operation])->Result<(image::RgbaImage,bool)>{let(image,profile)=decode(file)?;Ok((transform(image,operations)?.to_rgba8(),profile.is_some()))}
pub(crate) fn encode_rgba(image:image::RgbaImage,format:&str,quality:u8)->Result<Vec<u8>>{encode(&DynamicImage::ImageRgba8(image),None,format,quality)}
...
impl GalleryCatalog {
 ...
 put(&db,root,&node)?;rename_handle(&output,&destination)?;crate::privacy::inherit(&path,&destination)?;Ok(node.path)})();
 if result.is_err(){let _=delete_handle(&output);}result
 }
}
#[tauri::command]
pub async fn editor_preview(request:Request,core:State<'_,Arc<Core>>)->Result<Preview>{let privacy_epoch=crate::privacy::epoch();let privacy_result=(async {let permit=thumbnails::exclusive().await?;let root=root(&core)?;tauri::async_runtime::spawn_blocking(move||{let _permit=permit;preview(&root,request)}).await.map_err(err)?}).await;crate::privacy::finish(privacy_epoch,privacy_result)}
#[tauri::command]
pub async fn editor_export(request:Request,format:String,quality:u8,core:State<'_,Arc<Core>>,catalog:State<'_,Arc<GalleryCatalog>>,images:State<'_,Arc<ImageEngine>>)->Result<String>{let privacy_epoch=crate::privacy::epoch();let privacy_result=(async {let permit=thumbnails::exclusive().await?;let root=root(&core)?;let c=catalog.inner().clone();let i=images.inner().clone();tauri::async_runtime::spawn_blocking(move||{let _permit=permit;let origins=c.sync_origins(&root,i.list()?)?;let origin=origins.get(&request.query.target.file_id).cloned();c.export_edit(&root,request,format,quality,origin)}).await.map_err(err)?}).await;crate::privacy::finish(privacy_epoch,privacy_result)}
