...
impl Updater{
 ...
 fn begin(&self,phase:&str)->Result<()>{let mut d=self.data.lock().map_err(err)?;if matches!(d.status.phase.as_str(),"checking"|"downloading"|"installing"){return Err("update_busy".into());}d.status.phase=phase.into();d.status.error=None;d.status.received=0;d.status.total=0;self.cancel.store(false,Ordering::SeqCst);Ok(())}
